...
class DataFrameLoaders:
    def __init__(self, company_id, file_system_client: FileSystemClient) -> None:
        ...
        valid_substrings = ["employees/", "designations/"] # valid_substrings = ["employees/", "group_managements", "designations/"]
        self.company_files = self._list_all_files_by_company_id()
        self.data_path = [item for _, item in enumerate(self.company_files) if any(substring in item for substring in valid_substrings)]
        print (self.data_path)
        # self.data_path = ['designations/42/designations_ui42_20240614_115413.csv', 'employees/42/employees_ui42_20240614_115031.csv']
    ...
    def load_group_df(self,):
        # group_df = self._dataframe_from_adls_file_path('group_managements/group_managements.csv', self.file_system_client) # e
...
if __name__ == "__main__":
    ...
    emp_extractor = EmployeeIDExtractor(utility_db_file_system)
    print (emp_extractor.extract_employee_ids_from_department_id(65))
    print (emp_extractor.extract_employee_ids_from_role_id(142))
    print (emp_extractor.extract_employee_ids_from_group_id(30))
