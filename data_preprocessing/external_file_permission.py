...
class FilePermissions:
    ...
    def get_documnet_metadata(self, document_id: int, document_url: str):

        departement_pass = self.company_file_access_df[(self.company_file_access_df['fileId'] == document_id) & (self.company_file_access_df['type'] == "department")]['entityId'].to_list()
        role_pass        = self.company_file_access_df[(self.company_file_access_df['fileId'] == document_id) & (self.company_file_access_df['type'] == "role")]['entityId'].to_list()
        group_pass       = self.company_file_access_df[(self.company_file_access_df['fileId'] == document_id) & (self.company_file_access_df['type'] == "group")]['entityId'].to_list()

        metadata = {
            "doc_id": document_id,
            "document_url": document_url,
            "departement_ids": list(map(str, departement_pass)),
    ...
    def get_company_documents_metadata(self,):
        ...
        comapny_documents_metadata = []

        for file_id, urls in zip(company_file_ids, company_doc_urls):
            comapny_documents_metadata.append(self.get_documnet_metadata(file_id, urls))
        
        return comapny_documents_metadata
