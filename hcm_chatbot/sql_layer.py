...
def _build_rbac_system_prompt() -> str:
    ...
    return (
        ...
        "`SELECT departmentName, departmentHeadName, departmentHeadEmail FROM \`hcmatrix-utility-db\`.\`v_public_departments\` WHERE companyId = 1 AND (departmentName LIKE '%Business%' OR departmentLabel LIKE '%Business%') AND (departmentName LIKE '%Applications%' OR departmentLabel LIKE '%Applications%')`.\n"
