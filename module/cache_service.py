...
class LRUCache:
    ...
    Attributes:
        ...
        cache (OrderedDict): Stores key-value pairs along with their timestamps in the format 
            (key -> (value, timestamp)).
    
    Methods:
        get(key: int): Retrieves the value associated with a key if it exists and is not expired.
            Returns -1 otherwise.
        put(key: int, value: int): Adds or updates a key-value pair in the cache. Removes the least 
    ...
    def __init__(self, capacity: int, ttl: int = 86400):
        ...
        Args:
            ...
            ttl (int, optional): The time-to-live (TTL) for cache entries in seconds. 
