...
from rag_engine.utils import pdf_loader
...
def extract_document_metadata_and_permissions(company_id: str, utility_db_file_system: FileSystemClient):
    ...
    for metadata in docs_metadata:
        ...
        employee_ids_dep = list(
            itertools.chain(*[emp_extractor.extract_employee_ids_from_department_id(int(q)) for q in department_ids]))
        employee_ids_group = list(
            itertools.chain(*[emp_extractor.extract_employee_ids_from_group_id(int(k)) for k in group_ids]))
        employee_ids_role = list(
            itertools.chain(*[emp_extractor.extract_employee_ids_from_role_id(int(v)) for v in role_ids]))
